  ...
  sourceFile.forEachDescendant((node) => {
    if (node.isKind(SyntaxKind.CallExpression) && isToggleFunction(node)) {
      ...
      const featureName = featureNameProperty
        ...
        ?.getText()
        .slice(1, -1);
      const onFunction = onFunctionProperty?.getFirstChildByKind(SyntaxKind.ArrowFunction);
      const offFunction = offFunctionProperty?.getFirstChildByKind(SyntaxKind.ArrowFunction);
